                          ...
                          <div class="col-md-8">
                                <div class="card">
									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover edis2" style="width:100%;font-size:12px">
											...
											<?php											
											$no=0;										
											$query = mysqli_query($koneksi,"SELECT * FROM peskul WHERE kelas='$kelas' and eskul='$eskul' and semester='$semester' and tp='$tapel'");                                       
                                                ...
                                                <tr>
                                                    <td><?= $no; ?></td>                           
													 ...
													 <td><?= $data['kelas'] ?></td>
													  <td><?= $data['nilai'] ?></td>
													 <td>
													<a href="?pg=nileskul&ac=input&ids=<?= $data['nis'] ?>&e=<?= $eskul ?>&g=<?= $guru ?>" class="btn btn-sm btn-primary"><i class="material-icons">add</i></a>
					       ...
					       <div class="col-md-4">
								...
								<select name='smt'  class='form-select' required='true' style="width: 100%">
									...
									<label class="bold">Pilih Kelas</label>
                                     ...
                                     <select id='kelas'  class='form-select kelas' required='true' style="width: 100%">
								    	 <?php $kls = mysqli_query($koneksi, "SELECT * FROM kelas where kurikulum='1'"); ?>
                                ...
                                $('#simpan').click(function() {
                                    ...
                                    var guru = $('.guru').val();
									var eskul = $('.eskul').val();
                                    location.replace("?pg=nileskul&kelas=" + kelas + "&guru=" + guru + "&eskul=" + eskul);
                                }); 
                            </script>                                             
                                                </div>                                               
                                            </div>
                                ...
                                <div class="card">
									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover edis2" style="width:100%;font-size:12px">
											...
											<?php											
											$no=0;										
											$query = mysqli_query($koneksi,"SELECT * FROM siswa where nis='$ids'");                                        
											  while ($data = mysqli_fetch_array($query)) :
												...
												$('#formpeskul').submit(function(e){
													...
													$.ajax(
													{
														...
														success: function(data){
														 ...
														 setTimeout(function() {
                                                        window.location.replace('?pg=nileskul&kelas=<?= $kelas ?>&eskul=<?= $eskul ?>&guru=<?= $guru ?>');
                                                          }, 2000);
