...
$tp_list = [];
if ($mode !== 'all' && !empty($mapel_entries)) {
    $target_mapel_id = (int)$mapel_entries[0]['id'];
    $tingkat_tp = $class_level;
    if ($tingkat_tp === '' && $kelas !== '') {
        $kelasEscLevel = mysqli_real_escape_string($koneksi, (string)$kelas);
        $mapelEscLookup = mysqli_real_escape_string($koneksi, (string)$target_mapel_id);
        $rowJadwal = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT tingkat FROM jadwal_mapel WHERE kelas='$kelasEscLevel' AND mapel='$mapelEscLookup' LIMIT 1"));
        if ($rowJadwal && !empty($rowJadwal['tingkat'])) {
            $tingkat_tp = $rowJadwal['tingkat'];
        }
    }
    if ($tingkat_tp === '' && $kelas !== '') {
        ...
        if ($rowKelas && !empty($rowKelas['level'])) {
            $tingkat_tp = $rowKelas['level'];
        }
    }
    if ($tingkat_tp === '' && !empty($siswa_list[0]['kelas'])) {
        $kelasFallback = mysqli_real_escape_string($koneksi, (string)($siswa_list[0]['kelas']));
        $rowSiswaLevel = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT level FROM siswa WHERE kelas='$kelasFallback' LIMIT 1"));
        if ($rowSiswaLevel && !empty($rowSiswaLevel['level'])) {
            $tingkat_tp = $rowSiswaLevel['level'];
        }
    }
    $mapelEscTp = mysqli_real_escape_string($koneksi, (string)$target_mapel_id);
    $sqlTp = "SELECT tp, tujuan FROM tujuan WHERE mapel='$mapelEscTp'";
    if ($tingkat_tp !== '') {
        $levelEscTp = mysqli_real_escape_string($koneksi, (string)$tingkat_tp);
        $sqlTp .= " AND level='$levelEscTp'";
    }
    if ($semester !== '') {
        $semesterEscTp = mysqli_real_escape_string($koneksi, (string)$semester);
        $sqlTp .= " AND smt='$semesterEscTp'";
    }
    $sqlTp .= " ORDER BY tp ASC";
    if ($tpRes = mysqli_query($koneksi, $sqlTp)) {
        while ($tpRow = mysqli_fetch_assoc($tpRes)) {
            $code = trim((string)($tpRow['tp'] ?? ''));
            $desc = trim((string)($tpRow['tujuan'] ?? ''));
            if ($code === '') {
                $code = (string)(count($tp_list) + 1);
            }
            $tp_list[] = [
                'code' => $code,
                'description' => $desc,
...
if ($mode === 'all') {
    $sheet->setCellValue('A1', "FORMAT UPLOAD NILAI STS SEMUA MAPEL KELAS $kelas");
} else {
    $kode = $mapel_entries[0]['kode'] ?? 'MAPEL';
    $sheet->setCellValue('A1', "FORMAT UPLOAD NILAI STS $kode $kelas");
}
...
$tpCount = ($mode !== 'all') ? count($tp_list) : 0;
$tpStartColIndex = $mapelStartColIndex + $mapelCount;
$validationColIndex = null;
if ($mode !== 'all') {
    $validationColIndex = $tpStartColIndex + $tpCount;
    $lastColIndex = $validationColIndex;
} else {
...
if ($mode === 'all') {
    ...
    $sheet->setCellValue('D2', 'Nilai STS');
    $sheet->getStyle('D2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
} else {
    $nilaiColLetter = Coordinate::stringFromColumnIndex($mapelStartColIndex);
    $sheet->setCellValue($nilaiColLetter . '2', 'Nilai STS');
    $sheet->getStyle($nilaiColLetter . '2')->applyFromArray($styleHeader);
    if ($tpCount > 0) {
        $tpStartLetter = Coordinate::stringFromColumnIndex($tpStartColIndex);
        $tpEndLetter = Coordinate::stringFromColumnIndex($tpStartColIndex + $tpCount - 1);
        if ($tpCount > 1) {
            $sheet->mergeCells($tpStartLetter . '2:' . $tpEndLetter . '2');
        }
        $sheet->setCellValue($tpStartLetter . '2', 'Tingkat Ketercapaian TP');
        $sheet->getStyle($tpStartLetter . '2')->applyFromArray($styleHeader);
        if ($tpCount > 1) {
            $sheet->getStyle($tpStartLetter . '2:' . $tpEndLetter . '2')->applyFromArray($styleHeader);
        }
    }
    if ($validationColIndex !== null) {
        $validationLetter = Coordinate::stringFromColumnIndex($validationColIndex);
        $sheet->setCellValue($validationLetter . '2', 'Validasi');
        $sheet->getStyle($validationLetter . '2')->applyFromArray($styleHeader);
    }
}
...
if ($mode !== 'all') {
    for ($i = 0; $i < $tpCount; $i++) {
        $colIndex = $tpStartColIndex + $i;
        $colLetter = Coordinate::stringFromColumnIndex($colIndex);
        $tpItem = $tp_list[$i];
        $sheet->setCellValue($colLetter . '3', 'TP ' . $tpItem['code']);
        $sheet->setCellValue($colLetter . '4', 'TP|' . $mapel_entries[0]['id'] . '|' . $tpItem['code']);
        $sheet->getStyle($colLetter . '3')->applyFromArray($styleHeader);
    }
    if ($validationColIndex !== null) {
        $validationLetter = Coordinate::stringFromColumnIndex($validationColIndex);
        $sheet->setCellValue($validationLetter . '3', 'Status');
        $sheet->setCellValue($validationLetter . '4', 'VALIDASI');
        $sheet->getStyle($validationLetter . '3')->applyFromArray($styleHeader);
    }
}
...
foreach ($siswa_list as $siswa) {
    ...
    if ($mode !== 'all') {
        for ($i = 0; $i < $tpCount; $i++) {
            $colLetter = Coordinate::stringFromColumnIndex($tpStartColIndex + $i);
            $sheet->setCellValueExplicit($colLetter . $rowIndex, '', \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
            $sheet->getStyle($colLetter . $rowIndex)->applyFromArray($styleRow);
            $validation = $sheet->getCell($colLetter . $rowIndex)->getDataValidation();
            ...
            $validation->setPromptTitle('Tingkat Ketercapaian TP');
            $validation->setPrompt('Pilih TRUE jika TP tercapai, FALSE jika masih perlu peningkatan.');
            $validation->setShowErrorMessage(true);
            $validation->setErrorTitle('Input tidak valid');
            $validation->setError('Masukkan hanya TRUE atau FALSE.');
        }
        if ($validationColIndex !== null) {
            $validationLetter = Coordinate::stringFromColumnIndex($validationColIndex);
            if ($tpCount > 0) {
                $tpStartLetter = Coordinate::stringFromColumnIndex($tpStartColIndex);
                $tpEndLetter = Coordinate::stringFromColumnIndex($tpStartColIndex + $tpCount - 1);
                $tpRange = $tpStartLetter . $rowIndex . ':' . $tpEndLetter . $rowIndex;
                $columnsCount = $tpCount;
                $formula = '=IF(AND(' . $nilaiColLetter . $rowIndex . '<>"",COUNTBLANK(' . $tpRange . ')=0,' .
                    'SUMPRODUCT(--(UPPER(' . $tpRange . ')="TRUE")+--(UPPER(' . $tpRange . ')="FALSE"))=' . $columnsCount . '),"Valid","Tidak Valid")';
            } else {
                $formula = '=IF(' . $nilaiColLetter . $rowIndex . '<>"","Valid","Tidak Valid")';
            }
            $sheet->setCellValue($validationLetter . $rowIndex, $formula);
            $sheet->getStyle($validationLetter . $rowIndex)->applyFromArray($styleRow);
        }
    }
    $rowIndex++;
}
...
$notesStartRow = $rowIndex + 1;
if ($mode !== 'all' && $tpCount > 0) {
    $noteLastColLetter = Coordinate::stringFromColumnIndex($lastColIndex);
    $currentRow = $notesStartRow;
    $sheet->setCellValue('A' . $currentRow, 'KETERANGAN :');
    $sheet->mergeCells("A{$currentRow}:{$noteLastColLetter}{$currentRow}");
    $sheet->getStyle("A{$currentRow}:{$noteLastColLetter}{$currentRow}")->getFont()->setBold(true);
    $currentRow++;
    foreach ($tp_list as $tpItem) {
        $sheet->setCellValue('A' . $currentRow, '> TP ' . $tpItem['code'] . ': ' . $tpItem['description']);
        $sheet->mergeCells("A{$currentRow}:{$noteLastColLetter}{$currentRow}");
        $currentRow++;
    }
    $sheet->setCellValue('A' . $currentRow, 'Catatan Validasi:');
    $sheet->mergeCells("A{$currentRow}:{$noteLastColLetter}{$currentRow}");
    $sheet->getStyle("A{$currentRow}:{$noteLastColLetter}{$currentRow}")->getFont()->setBold(true);
    $currentRow++;
    $sheet->setCellValue('A' . $currentRow, '> Pastikan Nilai STS dan seluruh kolom TP terisi TRUE atau FALSE.');
    $sheet->mergeCells("A{$currentRow}:{$noteLastColLetter}{$currentRow}");
    $currentRow++;
    $sheet->setCellValue('A' . $currentRow, '> Status akan menjadi Valid jika semua kolom telah terisi dengan benar.');
    $sheet->mergeCells("A{$currentRow}:{$noteLastColLetter}{$currentRow}");
    $sheet->getStyle("A{$notesStartRow}:{$noteLastColLetter}{$currentRow}")
        ->getAlignment()
        ->setWrapText(true)
...
if ($mode === 'all') {
    $filename = 'Format_Nilai_STS_Semua_Mapel_' . preg_replace('/[^A-Za-z0-9_-]+/', '_', $kelas) . '.xlsx';
} else {
    $kode = $mapel_entries[0]['kode'] ?? 'MAPEL';
    $filename = 'Format_Nilai_STS_' . $kode . '_' . preg_replace('/[^A-Za-z0-9_-]+/', '_', $kelas) . '.xlsx';
}
