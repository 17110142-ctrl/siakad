...
?>           <?php if ($ac == '') : ?>
	        <div class="alert alert-dark" role="alert">
                  Asesmen Sumatif Tengah Semester <b>tidak wajib</b> untuk dilakukan, kewenangan diserahkan pada sekolah. 
 ...
 if (empty($_GET['mapel'])) {
	...
	$templateRoot = 'nilai/prosessts.php';
	if (!empty($homeurl ?? '')) {
		$templateRoot = rtrim($homeurl, '/') . '/mykurmer/nilai/prosessts.php';
	}
                          ...
                          <div class="col-md-8">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="bold"> SUMATIF TENGAH SEMESTER <span class="badge badge-secondary"><?= $mpl['kode'] ?> <?= $kelas ?></span></h5>
                                    ...
                                    <div class="card-body">
									<form id="form-nilai-sts" method="POST" action="nilai/crud_sts.php">
    ...
    <div class="card-box table-responsive">
        <table id="datatable1" class="table table-bordered table-hover edis2" style="width:100%;font-size:12px">
            ...
            <tbody>
                ...
                $tingkatTP = $tingkat;
                if ($tingkatTP === '' && $kelas !== '') {
                    $kelasEsc = mysqli_real_escape_string($koneksi, (string)$kelas);
                    $mapelEscTmp = mysqli_real_escape_string($koneksi, (string)$mapel);
                    $rowJadwal = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT tingkat FROM jadwal_mapel WHERE kelas='$kelasEsc' AND mapel='$mapelEscTmp' LIMIT 1"));
                    if ($rowJadwal && !empty($rowJadwal['tingkat'])) {
                        $tingkatTP = $rowJadwal['tingkat'];
                    }
                }
                if ($tingkatTP === '' && $kelas !== '') {
                    $kelasLookup = mysqli_real_escape_string($koneksi, (string)$kelas);
                    $rowKelas = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT level FROM kelas WHERE kelas='$kelasLookup' LIMIT 1"));
                    if ($rowKelas && !empty($rowKelas['level'])) {
                        $tingkatTP = $rowKelas['level'];
                    }
                }
                if ($tingkatTP === '' && $kelas !== '') {
                    $rowSiswa = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT level FROM siswa WHERE kelas='" . mysqli_real_escape_string($koneksi, (string)$kelas) . "' LIMIT 1"));
                    if ($rowSiswa && !empty($rowSiswa['level'])) {
                        $tingkatTP = $rowSiswa['level'];
                    }
                }
                if ($mapel !== '' && $tingkatTP !== '') {
                    $mapelEsc = mysqli_real_escape_string($koneksi, (string)$mapel);
                    $tingkatEsc = mysqli_real_escape_string($koneksi, (string)$tingkatTP);
                    $semesterEsc = mysqli_real_escape_string($koneksi, (string)$semester);
                    $qTP = mysqli_query($koneksi, "SELECT tujuan FROM tujuan WHERE mapel='$mapelEsc' AND level='$tingkatEsc' AND smt='$semesterEsc'");
                    if ($qTP) {
                        while ($rowTP = mysqli_fetch_assoc($qTP)) {
                            if (!empty($rowTP['tujuan'])) {
                                $TPList[] = $rowTP['tujuan'];
                            }
                        }
                ...
                if ($kelas !== '' && $mapel !== '') {
                    $kelasEsc = mysqli_real_escape_string($koneksi, (string)$kelas);
                    $mapelEsc = mysqli_real_escape_string($koneksi, (string)$mapel);
                    $desRes = mysqli_query($koneksi, "SELECT nis, tinggi, rendah FROM nilai_formatif WHERE kelas='$kelasEsc' AND mapel='$mapelEsc'");
                    if ($desRes) {
                        while ($rowDesc = mysqli_fetch_assoc($desRes)) {
                            $deskripsi_siswa[$rowDesc['nis']] = [
                                'tinggi' => $rowDesc['tinggi'] ?? '',
                                'rendah' => $rowDesc['rendah'] ?? ''
                            ];
                        }
                        mysqli_free_result($desRes);
                    }
                }
                $query = mysqli_query($koneksi, "SELECT * FROM siswa WHERE kelas='$kelas'"); 				
                while ($data = mysqli_fetch_array($query)) :
                    $sts = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM nilai_sts WHERE nis='$data[nis]' AND mapel='$mapel' AND khp='STS' and guru='$guru' and semester='$semester' and tp='$tapel'"));
                    $no++;
                    $deskripsi = $deskripsi_siswa[$data['nis']] ?? ['tinggi' => '', 'rendah' => ''];
                    $checkedTinggi = array_filter(array_map('trim', explode('||', $deskripsi['tinggi'] ?? '')));
                    $checkedRendah = array_filter(array_map('trim', explode('||', $deskripsi['rendah'] ?? '')));
                ?>
                    <tr>
                        <td><?= $no; ?></td>
                        <td><?= $data['nis'] ?></td>
                        <td><?= $data['nama'] ?></td>
                        <td>
                            <input type="number" name="nilai_sts[<?= $data['nis'] ?>]" class="form-control form-control-sm" style="width: 80px;" value="<?= $sts['nilai_sts'] ?>">
                        </td>
                        <td>
                            <?php if (!empty($TPList)) : ?>
                                <?php foreach ($TPList as $tujuan) :
                                    $tujuanTrim = trim((string)$tujuan);
                                    if ($tujuanTrim === '') { continue; }
                                    $isChecked = in_array($tujuanTrim, $checkedTinggi, true);
                                ?>
                                    <label class="d-block small">
                                        <input type="checkbox" name="tp_tercapai[<?= $data['nis'] ?>][]" value="<?= htmlspecialchars($tujuanTrim, ENT_QUOTES, 'UTF-8') ?>" <?= $isChecked ? 'checked' : '' ?>>
                                        <?= htmlspecialchars($tujuanTrim, ENT_QUOTES, 'UTF-8') ?>
                                    </label>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <?php if (trim((string)($deskripsi['tinggi'] ?? '')) !== '') : ?>
                                    <span class="small"><?= htmlspecialchars($deskripsi['tinggi'], ENT_QUOTES, 'UTF-8') ?></span>
                                <?php else : ?>
                                    <span class="text-muted small">TP belum diatur.</span>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if (!empty($TPList)) : ?>
                                <?php foreach ($TPList as $tujuan) :
                                    $tujuanTrim = trim((string)$tujuan);
                                    if ($tujuanTrim === '') { continue; }
                                    $isChecked = in_array($tujuanTrim, $checkedRendah, true);
                                ?>
                                    <label class="d-block small">
                                        <input type="checkbox" name="tp_kurang[<?= $data['nis'] ?>][]" value="<?= htmlspecialchars($tujuanTrim, ENT_QUOTES, 'UTF-8') ?>" <?= $isChecked ? 'checked' : '' ?>>
                                        <?= htmlspecialchars($tujuanTrim, ENT_QUOTES, 'UTF-8') ?>
                                    </label>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <?php if (trim((string)($deskripsi['rendah'] ?? '')) !== '') : ?>
                                    <span class="small"><?= htmlspecialchars($deskripsi['rendah'], ENT_QUOTES, 'UTF-8') ?></span>
                                <?php else : ?>
                                    <span class="text-muted small">TP belum diatur.</span>
                                <?php endif; ?>
                            <?php endif; ?>
    ...
    <div class="text-end">
        <button type="submit" class="btn btn-success">SIMPAN NILAI</button>
                                                     ...
                                                     <div class="modal-dialog modal-dialog-centered modal-lg">
                                                        <div class="modal-content">
                                                            <div class="modal-header justify-content-between align-items-center">
                                                                <h5 class="mb-0">NILAI STS <span class="badge badge-secondary"><?= $mapel !== '' ? (($mpl['kode'] ?? '') . ' ' . $kelas) : 'SEMUA MAPEL' ?></span></h5>
                                                                <div class="d-flex gap-2">
													<a href="<?= htmlspecialchars($perMapelHref, ENT_QUOTES, 'UTF-8') ?>" id="download-template-mapel" data-root="<?= htmlspecialchars($templateRoot, ENT_QUOTES, 'UTF-8') ?>" data-template="<?= htmlspecialchars($perMapelHref, ENT_QUOTES, 'UTF-8') ?>" class="<?= $perMapelClass ?>" target="_blank" rel="noopener noreferrer"><b>Download Format (Per Mapel)</b></a>
													<?php if ($is_admin_like): ?>
													<a href="<?= htmlspecialchars($allMapelHref, ENT_QUOTES, 'UTF-8') ?>" id="download-template-semua" data-root="<?= htmlspecialchars($templateRoot, ENT_QUOTES, 'UTF-8') ?>" data-template="<?= htmlspecialchars($allMapelHref, ENT_QUOTES, 'UTF-8') ?>" class="<?= $allMapelClass ?>" target="_blank" rel="noopener noreferrer"><b>Download Format (Semua Mapel)</b></a>
													<?php endif; ?>
                                                                </div>
                                                            </div>
							          ...
							          <form id="form-import" method="POST" action="nilai/crud_sts.php" enctype="multipart/form-data">
	...
	<div class="mb-3" id="import-kelas-wrapper"<?= $show_kelas_modal ? '' : ' style="display:none;"' ?>>
		...
		<select class="form-select" id="import-kelas-select">
			...
			<option value="<?= htmlspecialchars($kelasOpt) ?>" <?= $kelasOpt === $kelas ? 'selected' : '' ?>><?= htmlspecialchars($kelasOpt) ?></option>
													...
													<div id="progressbox-sts" class="mt-2"></div>
												</div>
                                                            </div>
					       ...
					       <div class="col-md-4">
							...
							<div class="col-md-12">
                                ...
                                $('#cari').click(function() {
                                    ...
                                    var mapel = $('.mapel').val();
									 var guru = $('.guru').val();
                                    location.replace("?pg=<?= enkripsi('sts') ?>&level=" + level + "&kelas=" + kelas + "&mapel=" + mapel + "&guru=" + guru);
                                }); 
                            </script>
                                            </div>
							...
							<script>
$('#form-nilai-sts').submit(function(e) {
    e.preventDefault();
    var data = $(this).serialize(); 
    $.ajax({
        type: 'POST',
        url: 'nilai/crud_sts.php',
        data: data,
        dataType: 'json',
...
$('#form-import').submit(function(e) {
								...
								$.ajax({
									...
									dataType: 'json',
									beforeSend: function() {
									$('#progressbox-sts').html('<div><label class="sandik" style="color:blue;">Data sedang diproses</label>&nbsp;&nbsp;&nbsp;<img src="../images/animasi.gif" style="width:50px;"></div>');
									},
									success: function(res) {
										...
										$('#progressbox-sts').html('<div class="alert alert-success">'+ msg +'</div>');
										setTimeout(function() {
											window.location.reload();
										}, 1500);
									},
									error: function(xhr) {
										...
										$('#progressbox-sts').html('<div class="alert alert-danger">'+ msg +'</div>');
									}
								})
								return false;
							});
...
(function(){
    ...
    const linkMapel = $('#download-template-mapel');
    const linkSemua = $('#download-template-semua');
	const mapelTemplateRoot = linkMapel.data('root') || 'mykurmer/nilai/prosessts.php';
    const semuaTemplateRoot = linkSemua.length ? (linkSemua.data('root') || mapelTemplateRoot) : mapelTemplateRoot;
    ...
    function updateTemplateLinks() {
        ...
        if (linkMapel.length) {
            if (shouldShowMapel) {
                ...
                const url = buildTemplateUrl(mapelTemplateRoot, {
                    m: mapelVal,
                    k: kelasVal,
                    s: semesterVal,
                    tp: tapelVal,
        ...
        if (linkSemua.length) {
            if (shouldShowSemua) {
                ...
                if (kelasSelected) {
                    const urlAll = buildTemplateUrl(semuaTemplateRoot, {
                        mode: 'all',
                        k: kelasVal,
                        s: semesterVal,
                        tp: tapelVal
    ...
    $(document).on('click', 'a.disabled', function(e){
        ...
        if (targetId === 'download-template-semua') {
