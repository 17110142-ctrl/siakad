...
<div style='background:#fff; width:97%; margin:0 auto; height:90%;'>
   ...
   <br>
		...
		<center><h4>JURNAL GURU BULAN <?= strtoupper($bulane['ket']) ?> <?= date('Y') ?><br><?= strtoupper($map['nama_mapel']) ?></h4></center>
		<br>
              ...
              <tr>
               ...
               <th style="text-align:center">HAMBATAN</th>
                <th style="text-align:center">PEMECAHAN</th>
				 ...
				 <?php
				 $no=0;
                $query = mysqli_query($koneksi, "select * from agenda WHERE kelas='$kelas' AND mapel='$mapel' AND bulan='$bl' and guru='$guru' and hambatan<>''");				 
...
$dompdf->render();
$dompdf->stream("Jurnal Guru Bulan ". $bl . ".pdf", array("Attachment" => false));
exit(0);
?>