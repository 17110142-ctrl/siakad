...
$simpan = insert($koneksi, 'materi', $data);
...
if ($simpan) {
    ...
    if (!$defer_wa && $setting && !empty($setting['url_api']) && $kelas_array) {
        ...
        while ($siswa = mysqli_fetch_assoc($result_siswa)) {
            $pesan = "*PEMBERITAHUAN MATERI BARU*\n\n" .
                     "Yth. Bapak/Ibu Orang Tua dari ananda " . $siswa['nama'] . ",\n\n" .
                     "Dengan ini kami memberitahukan bahwa ada materi baru:\n" .
    ...
    if ($want_json) {
        echo json_encode(['status' => 'error', 'message' => 'Gagal menyimpan data materi.']);
    } else {
        echo "Gagal menyimpan data materi.";
    }
}
?>
