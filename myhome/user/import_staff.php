...
if (isset($_FILES['file']['name'])) {
    ...
    if (in_array($extension, $ext)) {
        ...
        $sheetData = $spreadsheet->getActiveSheet()->toArray();
		 $exec = mysqli_query($koneksi, "delete from users WHERE level='staff'");
        for ($i = 3; $i < count($sheetData); $i++) {
           ...
           $qus = mysqli_query($koneksi, "SELECT username FROM users WHERE username='$username'");
            ...
            if ($cekuser == 0) {
                ...
                $exec = mysqli_query($koneksi, "INSERT INTO users(nip,nama,level,username,password,nowa) 
				VALUES ('$nip','$nama','staff','$username','$password','$nowa')");
