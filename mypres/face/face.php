  ...
  <link rel="stylesheet" type="text/css" href="<?= $homeurl ?>/face/style.css">
  
  <script src="<?= $homeurl ?>/face/jquery.min.js"></script>
  <script src="<?= $homeurl ?>/face/bootstrap.min.js"></script>
  <script src="<?= $homeurl ?>/face/face-api.js"></script>
  <style>
video {
  max-width: 100%;
  height: auto;
                          ...
                          <div class="col-md-8">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">DATA FACE RECOGNITION PEGAWAI</h5>
					       ...
					       <div class="col-md-4">
                                ...
                                <div class="card widget widget-payment-request">
                                    <div class="card-header">
										...
										<a href="?pg=face&ac=pegawai" class="btn btn-primary"><i class="material-icons">crisis_alert</i>Register</a>
                                    </div>
                                    <div class="card-body">
		      ...
		      <?php elseif ($ac == 'pegawai') : ?>			
					<div class="row">
                          <div class="col-md-7">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">REGISTRASI FACE RECOGNITION PEGAWAI</h5>
...
<script>
  ...
  $("#parent2").hide();
  Promise.all([
    faceapi.nets.faceRecognitionNet.loadFromUri('<?= $homeurl ?>/models'),
    faceapi.nets.faceLandmark68Net.loadFromUri('<?= $homeurl ?>/models'),
    faceapi.nets.ssdMobilenetv1.loadFromUri('<?= $homeurl ?>/models'),
    faceapi.nets.tinyFaceDetector.loadFromUri('<?= $homeurl ?>/models')
  ]).then(start)

  async function start() {
    $.ajax({
        datatype: 'json',
        url: "<?= $homeurl ?>/fetch.php",
        data: ""
        ...
        if(counter <= 5 && counter >= 0 ){
          ...
          if( detections != null){
            ...
            if(counter == 0){
              ...
              $.ajax({
                  type: "POST",
                  url: "<?= $homeurl ?>/ajax.php?pg=pegawai",
