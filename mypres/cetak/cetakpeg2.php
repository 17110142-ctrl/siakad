		 ...
		 <table class='it-grid it-cetak' width='100%'>       
              <tr>
                ...
                <th width="1%">A</th>
                  ...
                  <?php
			$query = mysqli_query($koneksi,"select id_user,nama,tugas from users WHERE tugas<>'' GROUP BY id_user");
             $no = 0;
              while ($peg = mysqli_fetch_array($query)) {
				$hadir= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idpeg='$peg[id_user]' AND ket='H' AND bulan='$bulan' AND tahun='$tahun' "));
				$sakit= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idpeg='$peg[id_user]' AND ket='S' AND bulan='$bulan' AND tahun='$tahun' "));
				$izin= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idpeg='$peg[id_user]' AND ket='I' AND bulan='$bulan' AND tahun='$tahun' "));
				$alpha= mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM absensi_les WHERE idpeg='$peg[id_user]' AND ket='A' AND bulan='$bulan' AND tahun='$tahun' "));
					  $no++;
			?>
							...
							<tr>
                                    ...
                                    <td>&nbsp;&nbsp;<?= ucwords(strtolower($peg['nama'])) ?></td>
									 <td>&nbsp;&nbsp;<?= ucwords(strtolower($peg['tugas'])) ?></td>
				<?php 
