...
ujian.id_ujian,ujian.pelanggaran ,ujian.status FROM nilai LEFT JOIN ujian ON nilai.id_ujian=ujian.id_ujian  where ujian.status='1' and nilai.id_siswa<>'' and nilai.id_ujian='$_GET[idu]' ORDER BY nilai.id_nilai DESC");
                                    ...
                                    <div class="card-body">
                                        <div class="widget-stats-container d-flex">
                                            ...
                                            <div class="widget-stats-content flex-fill">
												...
												<a href="?pg=<?= enkripsi('peserta') ?>&ac=login&idu=<?= $_GET['idu'] ?>" class="btn btn-sm btn-success kanan">Lihat</a>
...
<p style="color:red;">Tombol Ulang akan muncul jika nilai kurang dari KKM</p>
...
<div class='table-responsive'>
    ...
    <table id='datab' class='table table-bordered edis2' width="100%">
        ...
        <tbody id='logstatus'>
            ...
            ?>
                <tr>
					...
					<td>
					<?php if($nilai['nilai']< $setting['kkm']): ?>
					<h5><span class="badge badge-danger"><?= $nilai['nilai'] ?></span></h5>
					<?php else: ?>
					<h5><span class="badge badge-success"><?= $nilai['nilai'] ?></span></h5>
					<?php endif; ?>
					</td>
					<td><h5><span class="badge badge-dark"><?= $setting['kkm'] ?></span></h5></td>
                    <td>
					<?= $btn ?>
					<?= $btn2 ?>
					<?= $btn3 ?>
     ...
     $('#datab').on('click', '.kosongkan', function() {
			...
			$('#ulg').on('click', '.paksa', function() {
			
				Swal.fire({
					title: 'Yakin Selesaikan Ujian',
					text: "Peserta yang belum selesai akan dipaksa untuk selesai",
					type: 'warning',
					showCancelButton: true,
					confirmButtonColor: '#3085d6',
					cancelButtonColor: '#d33',
					...
					if (result.value) {
						$.ajax({
						url: 'siswa/tsiswa.php?pg=selesaikan',
