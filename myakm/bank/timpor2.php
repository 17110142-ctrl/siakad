...
$nomer=0;
  ...
  for ($i = 8; $i < count($sheetData); $i++) {
	        ...
	        $koneksi->query("INSERT INTO soal (id_bank,nomor,soal,jenis,pilA,pilB,pilC,pilD,pilE,perA,perB,perC,perD,perE,jawaban,max_skor)VALUES('$idmapel','$nomor','$soal','$jenis','$pilA','$pilB','$pilC','$pilD','$pilE','$perA','$perB','$perC','$perD','$perE','$jawab','$skor')");
		       $idsoal = $koneksi->insert_id;
		if ($jenis == '1') {	
				  $jawaban = $jawab;
				 $idjawab = $idsoal.'.1';
	              mysqli_query($koneksi,"INSERT INTO kunci_soal(jawaban,id_bank,id_soal,id_jawab,skor) VALUES ('$jawaban','$idmapel','$idsoal','$idjawab','$skor')");
				 }
			if ($jenis == '2') {	
				  $jawaban = $jawab;
				 $idjawab = $idsoal.'.1';	 
