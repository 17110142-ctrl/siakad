...
if ($pg == 'tambah') {
    ...
    $cek = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM banksoal WHERE  kode='$kode'"));
        ...
        $exec = mysqli_query($koneksi, "INSERT INTO banksoal (kode, nama,idguru,level,pk,model,status,soal_agama,groupsoal) VALUES ('$kode','$nama','$guru','$level','$pk','$model','$status','$agama','$groupsoal')");
			if ($exec) {
                echo "OK";
            } else {
...
if ($jenis == '3') {
   ...
   mysqli_query($koneksi,"INSERT INTO kunci_soal(jawaban,id_bank,id_soal,id_jawab,skor) VALUES ('$jawabQ[$i]','$id_bank','$idsoal','$idjawab','$skor')");
	...
	if ($jenis == '5') {
	   ...
	   $tskor = $jml_data * $skor;
	   $jawaban=implode(', ',$_POST['jawaban']);
		$koneksi->query(" INSERT INTO soal (id_bank,nomor,soal,max_skor,jenis,pilA,pilB,pilC,pilD,pilE,jawaban,perA,perB,perC,perD,perE,file,file1,fileA,fileB,fileC,fileD,fileE) VALUES ('$id_bank','$nomor','$isi_soal','$jml_data','5','$pilA','$pilB','$pilC','$pilD','$pilE','$jawaban','$perA','$perB','$perC','$perD','$perE','$urlx','$filex1','$filexA','$filexB','$filexC','$filexD','$filexE')");
	   	$idsoal = $koneksi->insert_id;	
		 mysqli_query($koneksi,"UPDATE soal SET max_skor='$tskor' WHERE id_soal='$idsoal'");
		for ($i=0; $i < $jml_data; $i++){
   $idjawab = $idsoal.'.'.($i+1);  
