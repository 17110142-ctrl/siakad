...
if($user['level']=='guru'):
$jbank = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM banksoal WHERE idguru='$id_user'"));
else:
...
?>           
			<?php if ($ac == '') : ?>
                          ...
                          <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header">
										...
										<a href="?pg=<?= enkripsi('banksoal') ?>&ac=tambah" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Buat Bank Soal"><i class="material-icons">add</i></a>   
												 ...
												 <div class="card-body">
													Keterangan :
													<p>Tersedia 5 Jenis Soal, Namun Jika hanya menggunakan 1 atau 2 Jenis Soal, Maka Jenis soal yang lain Kosongin saja<br>
												    File ZIP digunakan jika menggunakan sinkronisasi baik Android maupun Server Lokal</p>
												</div>
											</div>
										</div>
									</div>
											 ...
											 ?>
											   <?php while ($bank = mysqli_fetch_array($Q)) : ?>
												...
												<tr style="height:30px">
												 ...
												 <td><?= $bank['kode'] ?></td>
												 <td>												  
													 <h5><span class="badge badge-primary"><?= $bank['level'] ?></span> - <span class="badge badge-primary"><?= $bank['pk'] ?></span></h5>												 
												 </td>
												 ...
												 <td style="text-align:center">
												  ...
												  <a href="?pg=<?= enkripsi('banksoal') ?>&ac=edit&id=<?= enkripsi($bank['id_bank']) ?>"> <button class='btn btn-sm btn-primary' data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="material-icons">edit</i></button></a>
													...
													<?php if($jfile==0){ ?>
													<button data-id="<?= $bank['id_bank'] ?>" class="zip btn-sm btn btn-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Buat File Zip"><i class="fa fa-plus"></i> ZIP</button>
													<?php }else{ ?>
				...
				$('#datatable1').on('click', '.zip', function() {
						...
						swal({
							...
							{
								title: 'Sukses!',
								message: 'File Zip Berhasil di buat',
								titleColor: '#FFFF00',
								messageColor: '#fff',
			...
			<div class="row">          
				<div class="col-md-8">
				 <div class="card" id="blockui-card-1">
                   <div class="card">
                          <div class="card-body">
						<form id='formbank' class="row g-1">                         
                               <div class="col-md-5">
								<label class="form-label bold"> Model Soal</label>
								...
								<select name='pk' class='form-select' required='true' style="width: 100%">
								    <option value=''>Pilih Jurusan</option>
									 <option value='semua'>semua</option>
										<?php
										$qjur = mysqli_query($koneksi, "SELECT jurusan FROM siswa GROUP BY jurusan");
										while ($jur = mysqli_fetch_array($qjur)) {
							...
							<div class="col-md-5">
								<label class="form-label bold">Kode Bank</label>
							...
							<div class="col-md-6">
								<label class="form-label bold">Status Soal</label>
								<select name='status' class='form-select' required='true' style="width: 100%">
								  <option value='1'>Aktif</option>
							       ...
							       <div class='col-md-6'>
                                      <label class="form-label boldl">Soal Agama</label>
                     ...
                     <div class="col-xl-4">
                                    ...
                                    <div class="card-body" style="background-color:#fff;">
										...
										<?php 
										$Q = mysqli_query($koneksi, "SELECT * FROM banksoal ORDER BY id_bank DESC LIMIT 5");
										while ($data = mysqli_fetch_array($Q)) : 
										$mapel = fetch($koneksi,'mata_pelajaran',['kode'=>$data['nama']]);
		...
		$.ajax({
		type: "POST",
		url: "bank/tbanksoal.php?pg=ambil_kelas", 
		data: "level=" + level, 
		...
		$.ajax({
		type: "POST",
		url: "bank/tbanksoal.php?pg=ambil_mapel", 
		data: "pk=" + pk, 
	    success: function(response) { 
		$("#mapel").html(response);
				}
...
$mpl = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM mata_pelajaran WHERE kode='$bank[nama]'"));
  ...
  <div class="row">          
                   ...
                   <div class="card">
                     <div class="card-header">
                       <h5 class="card-title">EDIT BANK SOAL</h5>
						<div class="pull-right">
							<a href="?pg=banksoal" class="btn btn-secondary" data-bs-toggle="tooltip" data-bs-placement="top" title="Kembali"><i class="material-icons">arrow_back</i> Back</a>
									</div>
								</div>
							</div>
						</div>
          ...
          <div class="row">          
				<div class="col-md-8">
                   <div class="card">
                          <div class="card-body">
						<form id='formeditbank' class="row g-1"> 
							...
							<div class="col-md-12">
								...
								<select name='mapel' id='mapel' class='form-select' required='true' style="width: 100%">
								   <option value="<?= $bank['nama'] ?>"><?= $mpl['nama_mapel'] ?></option>
									<?php
									$mpl = mysqli_query($koneksi, "SELECT * FROM mata_pelajaran ORDER BY nama_mapel ASC");
                                ...
                                <div class="card widget widget-stats">
                                 ...
                                 <div class="card widget widget-list">
                                    ...
                                    <div class="card-body edis2">
                                        <span class="text-muted m-b-xs d-block"><?= $jbank ?> Bank Soal</span>
                                        <ul class="widget-list-content list-unstyled">
                                            <?php
							                if($user['level']=='guru'):
											$query = mysqli_query($koneksi, "SELECT * FROM banksoal WHERE idguru='$id_user' ORDER BY id_bank DESC LIMIT 4"); 
											 else:
											 $query = mysqli_query($koneksi, "SELECT * FROM banksoal  ORDER BY id_bank DESC LIMIT 4"); 
											
											endif;
											 while ($data = mysqli_fetch_array($query)) :
											  $jsoal = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM soal WHERE id_bank='$data[id_bank]'"));
											   ?>
											<li class="widget-list-item widget-list-item-green">
                                                <span class="widget-list-item-icon"><i class="material-icons-outlined">quiz</i></span>
                                                <span class="widget-list-item-description">
                                                    <a href="#" class="widget-list-item-description-title">
                                                        <?= $data['kode'] ?> Jml Soal <?= $jsoal ?>
                                                    </a>
                   ...
                   <div class="card">
                    ...
                    <button class="nav-link" id="pills-MULTI-tab" data-bs-toggle="pill" data-bs-target="#pills-MULTI" type="button" role="tab" aria-controls="pills-MULTI" aria-selected="false">PG MULTI</button>
                     ...
                     <button class="nav-link" id="pills-BS-tab" data-bs-toggle="pill" data-bs-target="#pills-BS" type="button" role="tab" aria-controls="pills-BS" aria-selected="false">BENAR SALAH</button>
						 ...
						 <li class="nav-item" role="presentation">
                     <button class="nav-link" id="pills-JODOH-tab" data-bs-toggle="pill" data-bs-target="#pills-JODOH" type="button" role="tab" aria-controls="pills-JODOH" aria-selected="false">MRNJODOHKAN</button>
                          </li>  
						   <?php } ?>
						  <li class="nav-item" role="presentation">
