                          ...
                          <div class="col-md-8">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="bold">NILAI SIKAP <span class="badge badge-primary">SPIRITUAL</span>
										<?php if($kelas<>''){ ?>
									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover edis2" style="width:100%;font-size:12px">
											...
											$query = mysqli_query($koneksi, "SELECT * FROM siswa WHERE kelas='$kelas'");
											 ...
											 $des = fetch($koneksi,'spiritual',['nis'=>$data['nis'],'mapel'=>$mapel,'smt'=>$semester,'guru'=>$guru]);	
											$sos = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM spiritual WHERE nis='$data[nis]' and smt='$semester' and tp='$tapel'"));
											$no++;
											   ?>
                                                <tr>
                                                    <td><?= $no; ?></td>
													 ...
													 <td><?= $data['nama'] ?></td>
													  ...
													  <td> 
														<?php if($sos==0): ?>
													<a href="?pg=<?= enkripsi('nsikap') ?>&ac=<?= enkripsi('edit') ?>&id=<?= enkripsi($data['id_siswa']) ?>&k=<?= enkripsi($kelas) ?>&m=<?= enkripsi($mapel) ?>&g=<?= enkripsi($guru) ?>" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Input Nilai"><i class="material-icons">edit</i> </a>																								
													<?php else : ?>
                                ...
                                <div class="card widget widget-payment-request">
                                    <div class="card-header">                                    
										<h5 class="card-title">NILAI SIKAP SPIRITUAL</h5>									
									</div>
                                    <div class="card-body">
							...
							<div class="col-md-12">
								...
								<select name="guru" class='form-select guru' required='true' style="width: 100%" >
										...
										$('#cari').click(function() {
											...
											var mapel = $('.mapel').val();
											 var guru = $('.guru').val();
											location.replace("?pg=<?= enkripsi('nsikap') ?>&level=" + level + "&kelas=" + kelas + "&mapel=" + mapel + "&guru=" + guru);
										}); 
									</script>
                                            </div>
			 ...
			 ?>
	                ...
	                <div class="row">
                          <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="bold">NILAI SIKAP KI-1 <span class="badge badge-secondary"><?= $siswa['nama'] ?></span></h5>
                                        ...
                                        <table id="datatable1" class="table table-bordered edis2" style="width:100%;font-size:12px">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
													<select name='ket1'  class='form-select' required='true' style="width:100%;">
											 <option value=''>Pilih Sikap</option>
											  
												 <?php $quem = mysqli_query($koneksi, "SELECT * FROM m_spiritual"); ?>
													 <?php while ($m = mysqli_fetch_array($quem)) : ?>
														 <option value="<?= $m['ket'] ?>"><?= $m['ket'] ?></option>
																<?php endwhile ?>
																	</select>
													</td>                                              
														...
														<select name='ket2'  class='form-select' required='true' style="width:100%;">
														 <option value=''>Pilih Sikap</option>
															 ...
															 <?php $quem = mysqli_query($koneksi, "SELECT * FROM m_spiritual ORDER BY id DESC"); ?>
																 <?php while ($m = mysqli_fetch_array($quem)) : ?>
																	 <option value="<?= $m['ket'] ?>"><?= $m['ket'] ?></option>
																			<?php endwhile ?>
																				</select>
							...
							<script>
								$('#formmapel3').submit(function(e){
									...
									width: "30%"
										...
										setTimeout(function()
										{
										   window.location.replace('?pg=<?= enkripsi(nsikap) ?>&mapel=<?= $mapel ?>&kelas=<?= $kelas ?>&guru=<?= $guru ?>');
										}, 2000);
										 } else {
										 iziToast.warning(
										{
