	...
	<?php if ($ac == '') : ?>
                   <div class="row">
                          <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">INPUT KOMPETENSI PENGETAHUAN</h5>
									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover edis2" style="width:100%;font-size:12px">
											...
											while ($data = mysqli_fetch_array($query)) :
											 ...
											 $jumdes = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM deskripsi where mapel='$data[mapel]' and level ='$data[tingkat]' and guru='$data[guru]' and ki='KI3' and smt='$setting[semester]'"));
											  $no++; 
											   ?>
                                                <tr>
                                                    <td><?= $no; ?></td>
													...
													<td><a href="?pg=<?= enkripsi('ki3') ?>&ac=<?= enkripsi('input') ?>&l=<?= enkripsi($data['tingkat']) ?>&m=<?= enkripsi($data['mapel']) ?>&g=<?= enkripsi($data['guru']) ?>" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Tambah Materi"><i class="material-icons">select_all</i> </a></td>
                                                </tr>
												<?php endwhile; ?>
												</tbody>
                                                </table>
					...
					<?php elseif ($ac == enkripsi('input')): ?>
                    	<?php
							   ...
							   $jml = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM deskripsi where mapel='$mapel' and level ='$tingkat' and guru='$guru' and ki='KI3' and smt='$setting[semester]'"));
							   $jumlah = $jml + 1;
                          ...
                          <div class="col-md-8">
                                <div class="card">
									...
									<div class="card-box table-responsive">
                                        <table id="datatable1" class="table table-bordered table-hover" style="width:100%;font-size:12px">
											...
											<?php
											$no=0;
											$query = mysqli_query($koneksi, "SELECT * FROM deskripsi where guru='$guru' and mapel='$mapel' and level='$tingkat' and smt='$setting[semester]' and ki='KI3'"); 
											while ($data = mysqli_fetch_array($query)) :													  
											$no++;
											   ?>
                                                <tr>
													...
													<td><?= $data['deskripsi'] ?></td>
													  <td>
											<a href="?pg=<?= enkripsi('ki3') ?>&ac=<?= enkripsi('edit') ?>&id=<?= enkripsi($data['id']) ?>" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit LM"><i class="material-icons">edit</i> </a>											
					       ...
					       <div class="col-md-4">
                                ...
                                <div class="card widget widget-payment-request">
                                    <div class="card-header">
                                       <h5 class="bold">KOMPETENSI PENGETAHUAN</h5>
									...
									<input type="hidden" name="jumlah" value="<?= $jumlah ?>" >
									  ...
									  <div class="input-group mb-1">
                                         <input type="text" name="kd" value="3.<?= $jumlah; ?>" class="form-control" readonly >
                                        </div>
                 ...
                 <script>
						$('#formdeskrip').submit(function(e) {
								...
								$.ajax({
									type: 'POST',
									 url: 'kurtilas/tdeskrip.php?pg=deskrip3',
			 ...
			 ?>					 
                 ...
                 <script>
						$('#formedit').submit(function(e) {
								...
								$.ajax({
									type: 'POST',
									 url: 'kurtilas/tdeskrip.php?pg=edit_deskrip3',
									enctype: 'multipart/form-data',
									data: data,
									cache: false,
									contentType: false,
									...
									success: function(data) {
										...
										setTimeout(function() {
											window.location.replace('?pg=<?= enkripsi(ki3) ?>&ac=<?= enkripsi(input) ?>&l=<?= enkripsi($dataz[level]) ?>&m=<?= enkripsi($dataz[mapel]) ?>&g=<?= enkripsi($dataz[guru]) ?>');
										}, 2000);
									}
								})
								return false;
