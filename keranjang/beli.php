...
while ($data = mysqli_fetch_array($query)) :
	...
	$simpan = mysqli_query($koneksi,"INSERT INTO transaksi(tanggal,idsiswa,idproduk,jumlah,harga,total_harga,status) 
	VALUES('$tanggal','$ids','$idp','$jumlah','$harga','$total','0')");
	if($simpan){
		mysqli_query($koneksi,"DELETE FROM keranjang WHERE idsiswa='$id_siswa'");
		$trx = mysqli_fetch_array(mysqli_query($koneksi, "SELECT idsiswa,SUM(total_harga) AS total FROM transaksi  WHERE idsiswa='$id_siswa' AND status='0'"));
	$exec = mysqli_query($koneksi,"INSERT INTO invoice(tanggal,idsiswa,nama,total,reff) VALUES('$tanggal','$id_siswa','$siswa[nama]','$trx[total]','$reff')");
	    if($exec){
			mysqli_query($koneksi,"UPDATE transaksi SET status='1' WHERE idsiswa='$id_siswa' AND status='0'");
